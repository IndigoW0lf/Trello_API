...
def create_card(list_id, name, desc=None):
    ...
    print(f"Creating card with name: {name}, in list: {list_id}")
    try:
...
def create_checklist(card_id, name):
    ...
    print(f"Creating checklist with name: {name}, on card: {card_id}")
    try:
        response = requests.post(url, params=query)
        response.raise_for_status()
    except requests.exceptions.RequestException as e:
...
def create_checklist_item(checklist_id, name):
    ...
    print(f"Creating checklist item with name: {name}, on checklist: {checklist_id}") 
    try:
        response = requests.post(url, params=query)
        response.raise_for_status()
    except requests.exceptions.RequestException as e:
